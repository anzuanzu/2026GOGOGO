...
const XINBAN_SALESPERSON_COLLECTION = 'salespersons_config';
...
function setupBranchRMListener(branchName) {
    try {
        ...
        // 新板分行使用 salespersons_config 集合（與管理面板寫入方式一致）
        if (branchName === 'xinban') {
            rmUnsubscribers[branchName] = onSnapshot(collection(db, XINBAN_SALESPERSON_COLLECTION), (querySnapshot) => {
                console.log(`${branchName} 分行收到 RM 目標更新 (salespersons_config)`);

                const salesTargets = {};
                querySnapshot.forEach((doc) => {
                    const data = doc.data();
                    if (data.name && data.target !== undefined) {
                        salesTargets[data.name] = data.target;
                    }
                });

                if (Object.keys(salesTargets).length > 0) {
                    branchData[branchName].salesTargets = salesTargets;
                    console.log(`${branchName} 分行 RM 目標已更新:`, Object.keys(salesTargets).length, '位理專');
                } else {
                    console.log(`${branchName} 分行 salespersons_config 為空，使用預設值`);
                }
            ...
            rmUnsubscribers[branchName] = onSnapshot(configQuery, (querySnapshot) => {
                ...
                console.error(`${branchName} 分行監聯 RM 目標時出錯:`, error);
                showToast(`${branchName} 分行 RM 目標同步出錯`, true);
            });
        }
...
async function loadBranchRMTargetsOnce(branchName) {
    try {
        ...
        // 新板分行使用 salespersons_config 集合
        if (branchName === 'xinban') {
            const querySnapshot = await getDocs(collection(db, XINBAN_SALESPERSON_COLLECTION));
            ...
            if (Object.keys(salesTargets).length > 0) {
                branchData[branchName].salesTargets = salesTargets;
                console.log(`${branchName} 分行 RM 目標載入成功 (salespersons_config):`, Object.keys(salesTargets).length, '位理專');
            } else {
                console.log(`${branchName} 分行 salespersons_config 為空，使用預設值`);
            }
        } else {
...
function updateBranchCard(branchName, stats) {
    ...
    // 動態更新理專人數
    const rmCountEl = document.getElementById(`${branchName}-rm-count`);
    if (rmCountEl) {
        const rmCount = Object.keys(branchData[branchName].salesTargets).length;
        rmCountEl.textContent = rmCount;
    }
...
function updateMonthlyStats() {
    ...
    Object.keys(monthIdMap).forEach(monthKey => {
        ...
        if (stats) {
            ...
            // 更新分行明細（如果存在）
            const banqiaoEl = document.getElementById(`${prefix}-banqiao`);
            const huajiangEl = document.getElementById(`${prefix}-huajiang`);
            const xinbanEl = document.getElementById(`${prefix}-xinban`);
            if (banqiaoEl) banqiaoEl.textContent = `${stats.branches.banqiao.toLocaleString()}萬`;
            if (huajiangEl) huajiangEl.textContent = `${stats.branches.huajiang.toLocaleString()}萬`;
            if (xinbanEl) xinbanEl.textContent = `${stats.branches.xinban.toLocaleString()}萬`;
        }
    });
}
